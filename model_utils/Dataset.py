...
class Dataset(ABC):
    ...
    def balance_dataset(self, max_num_samples=sys.maxsize):
        ...
        for class_num in range(num_classes):
            new_class_data, new_class_labels = self._oversample(self._train_X, self._train_Y, class_num,
                                                                goal_num_samples)
        ...
        if max_num_samples < sys.maxsize:
            ### Under Sampling ###
            for class_num in range(num_classes):
                self._train_X, self._train_Y = self._undersample(self._train_X, self._train_Y, class_num,
                                                                 max_num_samples)
            self._train_X, self._train_Y = self._undersample_all_negative_labels(self._train_X, self._train_Y,
                                                                                 max_num_samples)
...
class TextClassificationDataset(Dataset):
    def __init__(self, data_file_path, text_column_name=None, label_column_name=None, seed=SEED, validation_set_size=0):
        ...
        labels = df.loc[:,
                 'TrIP':'PIP'].to_numpy()  # **** Needs to be a 2d list, list of lists containing 8 0's or 1's indicating relation *****
...
class CategoricalTextClassificationDataset(Dataset):
    def __init__(self, data_file_path, text_column_name=None, label_column_name=None, seed=SEED, validation_set_size=0):
        ...
        if (text_column_name is None or label_column_name is None):
            text_column_name = 'text'
            label_column_name = 'label'
            df = pd.read_csv(data_file_path, header=None, names=[text_column_name, label_column_name], delimiter='\t',
...
class BinaryTextClassificationDataset(Dataset):
    ...
    def __init__(self, data_file_path, text_column_name=None, label_column_name=None, label_column_names=None,
                 seed=SEED, validation_set_size=0):
        '''
        ...
        # load the labels
        if (
                label_column_names is None):  # check if binary or multi-label #TODO --- this isn't necessarily true. I think I should just load the data differently for multilabel or multi-class problems (create a different method)
            ...
            if (text_column_name is None or label_column_name is None):
                ...
                label_column_name = 'label'
                df = pd.read_csv(data_file_path, header=None, names=[text_column_name, label_column_name],
                                 delimiter='\t', quoting=csv.QUOTE_NONE)  # .dropna()
            else:
                df = pd.read_csv(data_file_path, delimiter='\t', quoting=csv.QUOTE_NONE)  # .dropna()
...
class TokenClassificationDataset(Dataset):
    ...
    def __init__(self, data_file_path, num_classes, multi_class, tokenizer, seed=SEED, validation_set_size=0,
                 max_num_tokens=512,
                 shuffle_data=True):
    ...
    def preprocess(self, input_file):
        ...
        def tokenize_sample(df_sample):
            ...
            if (len(tokenized['input_ids'][0]) != len(new_labels)):
                print(
                    f"MISMATCH: {len(tokenized['input_ids'][0])}, {len(new_labels)}, {tokenized['input_ids']}, {df_sample['text']}")
                exit()
            # else:
            #    print("MATCHED")
...
class n2c2RelexDataset(TextClassificationDataset):
    def __init__(self, data_file_path, labels_file_path, text_column_name=None, label_column_name=None, seed=SEED,
    ...
    def make_dataframe(self, data_file_path, labels_file_path):
        ...
        with open(labels_file_path, 'r') as yfile:
            ...
            dfy = {'Strength-Drug': [], 'Form-Drug': [], 'Dosage-Drug': [], 'Duration-Drug': [], 'Frequency-Drug': [],
                   'Route-Drug': [], 'ADE-Drug': [], 'Reason-Drug': []}
            count = 0  # count is used to make the for loop skip the first line (which is a string)
            for row in reader:
                if count != 0:
