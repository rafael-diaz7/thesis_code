...
for ind, (q, c) in enumerate(df[["question", "context"]].values):
    c = tokenizer.decode(tokenizer(c, truncation=True, max_length=3900)["input_ids"])
    predictions.append(llm(prompt.format(question=q, context=c)).split("\n")[-1].strip())
    if ind % 100 == 0:
...
predictions_df.to_csv("llama_context_predictions.csv", index=False)